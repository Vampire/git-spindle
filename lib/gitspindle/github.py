...
#            error("You should not have capital letters in your repository name, please rename it from %s to %s" % (repo.name, repo.name.lower()))
    ...
    def edit_hook(self, opts):
        ...
        elif opts['<name>'].startswith('web-') and opts['<name>'][4:].isdigit():
            id = int(opts['<name>'][4:])
    ...
    def gists(self, opts):
        ...
        for gist in self.gh.iter_gists(user):
            description = gist.description or 'Files: %s' % ', '.join([x.name for x in gist.iter_files()])
            print("%s%s - %s" % (gist.html_url, '' if gist.is_public() else ' (secret)', description))

...
__git_spindle() {
    ...
    __git_spindle_set_previous_args $1 previous_args valued_options
...
_git_spindle_add_hook() {
    ...
    case ${#previous_args[@]} in
        1)
            ...
            local IFS=$'\n\r'
            local -a hooks=($(DEBUG=1 __git hub available-hooks 2>/dev/null))
...
_git_spindle_apply_pr() {
    ...
    case "$1,${GIT_SPINDLE_COMPLETE_REMOTE-no}" in
        ...
        bb,*)
            local IFS=$'\n'
            local -a issues=($(__git bb issues 2>/dev/null))
...
_git_spindle_ignore() {
    ...
    local IFS=$'\n\r'
    local -a ignore_languages=($(__git hub ignore))
...
_git_spindle_issue() {
    ...
    local IFS=$'\n\r'
    local -a issues=($(__git $1 issues $repo 2>/dev/null))
    issues=("${issues[@]##Issues *}")
    issues=("${issues[@]##Merge *}")
    issues=("${issues[@]##Pull *}")
    issues=("${issues[@]#[}")
...
_git_spindle_merge_request() {
    ...
    case "$cur" in
        *:*:*)
            ;;
        *:*)
            case " $(echo $(__git remote)) " in
                *\ upstream\ *)
                    __gitcomp_nl "$(__git for-each-ref --format='%(refname:strip=3)' refs/remotes/upstream)" "" "${cur#*:}"
                    ;;
                *)
                    __gitcomp_nl "$(__git for-each-ref --format='%(refname:strip=3)' refs/remotes/origin)" "" "${cur#*:}"
                    ;;
            esac
            ;;
        *)
...
_git_spindle_pull_request() {
    case "$1,$prev" in
        hub,--issue)
            ...
            local IFS=$'\n\r'
            local -a issues=($(__git hub issues --parent 2>/dev/null || __git hub issues 2>/dev/null))
            issues=("${issues[@]##Issues *}")
            issues=("${issues[@]##*/pull/*}")
            issues=("${issues[@]#[}")
            __gitcomp_nl_append "${issues[*]%%]*}"
...
_git_spindle_remove_collaborator() {
    ...
    local IFS=$'\n\r'
    local -a issues=($(__git hub collaborators 2>/dev/null))
    __gitcomp_nl_append "${issues[*]%%]*}"
}
...
_git_spindle_remove_deploy_key() {
    ...
    local IFS=$'\n'
    local -a deploy_keys=($(__git $1 deploy-keys 2>/dev/null))
...
_git_spindle_remove_member() {
    ...
    local IFS=$'\n'
    local -a members=($(__git lab members 2>/dev/null))
    members=("${members[@]%% \(*}")
    IFS=' '
    __gitcomp "${members[*]#* }"
}
...
_git_spindle_remove_privilege() {
    ...
    local IFS=$'\n'
    local -a members=($(__git bb privileges 2>/dev/null))
    members=("${members[@]%% \(*}")
    IFS=' '
    __gitcomp "${members[*]#* }"
}
...
__git_spindle_set_previous_args() {
    __git_spindle_set_previous_args_or_options $1 args "$2" $3
}

__git_spindle_set_previous_options() {
    __git_spindle_set_previous_args_or_options $1 options "$2" $3
}

__git_spindle_set_previous_args_or_options() {
    local prev_word dummy spindle_passed=0
    for word in "${words[@]:1:$cword-1}"; do
        # skip everything until the spindle sub-command
        # everything up to it are common git options and
        # not interesting for the completion functions here
        if [ $spindle_passed -eq 0 ]; then
             [ "$word" = "$1" ] && spindle_passed=1
             continue
        fi

        case "$prev_word,$4, $(eval "echo \${${4-dummy}[@]}") " in
            --account,*|*,?*,*\ $prev_word\ *)
                [ $2 = options ] && eval "$3[\${#$3[@]}]=$word"
                prev_word=
                continue
                ;;
        esac
...
__git_spindle_set_origin() {
    case "$prev" in
        --upstream-branch)
            unset COMPREPLY
            __gitcomp "$(__git for-each-ref --format '%(refname:strip=3)' refs/remotes/upstream/)"
            return 1
            ;;
        *)
            __git_spindle_protocols $1
...
__git_spindle_repos() {
    ...
    case "$1,$cur" in
        ...
        *,*/*)
            repos=($(__git $1 repos "${cur%/*}" 2>/dev/null))
            repos=(${repos[@]%% *})
        ...
        *)
            ...
            if [ ${2-also_own} = also_own ]; then
                repos=($(__git $1 repos 2>/dev/null))
                repos=(${repos[@]%% *})
...
__git_spindle_hooks() {
    ...
    local IFS=$'\n'
    hooks=($(__git hub hooks 2>/dev/null))
    hooks=(${hooks[@]%% \(*})
    hooks=(${hooks[@]## *})
    ...
    case "$hook_name,${GIT_SPINDLE_COMPLETE_REMOTE-no}" in
        ...
        *)
            case "$cur" in
                events=*,*)
                    local IFS=$'\n\r'
                    local -a hook_settings=($(DEBUG=1 __git hub available-hooks $hook_name 2>/dev/null))
                    unset IFS
                    local prefix="${cur#events=}"
...
# for compatibility to Git < 2.13.0
declare -f __git >/dev/null || __git() {
    git ${__git_dir:+--git-dir="$__git_dir"} "$@"
}
